...
#         return "Please provide both the sum of money to invest and the number of asset classes to invest in."

#     if number_of_stocks < 1 or number_of_stocks > 5:
#         return "The number of asset classes must be between 1 and 5."
...
@tool("InvestmentTool", return_direct=False)
def InvestmentTool(sum_of_money: float = None, api_key: str = None) -> dict:
    """
    Use this tool to advise the user on how to allocate their money into the top-performing asset classes.
    The tool will return a dictionary with a percentage distribution across all asset classes,
    based on the rankings from the MarketAnalysisTool.
    """
    global investment_distribution, distributionDone
    api_key = "KYpAilMplU1cVSK0H7N1P5OoR2znLbsIJa9yOER1" 

    # Check if sum of money is provided
    if sum_of_money is None:
        return "Please provide the sum of money to invest."

    # Fetch market performance rankings using MarketAnalysisTool
    #market_tool = MarketAnalysisTool(api_key=api_key)
    rankings = market_analysis_tool.invoke({"api_key": api_key})

    # Handle case where rankings might not be available
    if not rankings:
        return "Unable to fetch market data. Please try again later."

    # We'll use ALL available asset classes in the rankings
    selected_rankings = sorted(rankings.items(), key=lambda x: x[1]['Score'], reverse=True)

    # Distribute the sum of money based on the scores
    total_score = sum(ranking['Score'] for _, ranking in selected_rankings)
    investment_distribution = {
        category: round((ranking['Score'] / total_score) * sum_of_money, 2)
        for category, ranking in selected_rankings
    }
...
For general advice:
If user asks for general advice, you must use the market_analysis_tool to analyze the market and rank asset classes based on current performance. 
You must never ask a user for profile information in the case where the user ask for general investment advice.

1. You must use the market_analysis_tool to analyze the market and rank asset classes based on current performance.
...
You must use the InvestmentTool to advise the user on how to allocate their money into a given number of asset classes.
...
def setup_chain():
    ...
    tools = [InvestmentTool, ProfileTool,market_analysis_tool]
...
# Custom function to format the dictionary into a more readable string
def format_rankings(rankings: dict) -> str:
    formatted_output = "Market Rankings:\n"
    for rank, details in rankings.items():
        formatted_output += f"Rank {rank}: {details['Category']} - Score: {details['Score']:.2f}\n"
...
async def handle_message(message: cl.Message):
    ...
    # Assuming the output contains the ranking in result["output"], you can format it
    if isinstance(result["output"], dict):
        output_content = format_rankings(result["output"])  # Use the custom formatter
    else:
        output_content = result["output"]  # If it's not a dictionary, leave it as is
    ...
    # Check if user is asking for general advice
    if 'general' in user_message.lower():
        # Fetch market rankings using MarketAnalysisTool
        rankings = market_analysis_tool.invoke({"api_key": api_key})
        
        # Distribute funds based on market rankings
        sum_of_money = 10000  # Example sum, could be retrieved from user input
        investment_distribution = InvestmentTool.invoke({"sum_of_money": sum_of_money, "api_key": api_key})
        distributionDone = True

    # Personalized advice case (when profile is involved)
    elif 'profile' in user_message.lower():
        # Here you could gather profile info using ProfileTool
        profile_complete = True
